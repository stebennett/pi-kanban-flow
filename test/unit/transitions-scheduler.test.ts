...
const pr = (state: "open" | "merged" = "open") => ({ number: 1, url: "https://github.com/owner/repo/pull/1", head: "kanban/card/CARD-0001-example", base: "main" as const, state, operation_id: metadata.operationId, head_commit: "a".repeat(40), merge_commit: state === "merged" ? "b".repeat(40) : null, last_checked_at: at });
...
test("design, split, implementation, review, ship and merge transitions enforce evidence", () => {
  ...
  current = applyTransition(current, request({ kind: "design_passed", evidence: { branch: "kanban/design/CARD-0001-example", producerResultPath: "docs/cards/artifacts/CARD-0001/design-producer-x.yaml", checkerResultPath: "docs/cards/artifacts/CARD-0001/design-check-x.yaml", pr: pr() } })).snapshot;
  assert.equal(current.cards[0].status, "design_review");
  current = applyTransition(current, request({ kind: "design_merged", evidence: { pr: { ...pr("merged"), head_commit: "a".repeat(40) }, approvedCommit: "a".repeat(40) } })).snapshot;
