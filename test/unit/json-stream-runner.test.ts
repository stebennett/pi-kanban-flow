...
test("Pi 0.85 events may attest parent-resolved thinking and end with agent_settled", () => { const evaluator = new JsonRunEvaluator({ ...expectation, plannedThinking: "medium" }, 20); const values = events(); delete values[2].message.thinkingLevel; values.push({ type: "agent_settled" }); values.forEach((event) => evaluator.accept(event)); assert.equal(evaluator.finish(0).thinking, "medium"); });
