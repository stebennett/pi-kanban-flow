...
export function buildChildAttestation(plan: DispatchPlan, success: DispatchSuccess, identity: ExecutionIdentity, normalization: NormalizationInput, findingIds: readonly string[] = []): Record<string, unknown> {
  ...
  if (!Value.Check(ArtifactAttestationSchema, value)) {
    const paths = [...Value.Errors(ArtifactAttestationSchema, value)].slice(0, 5).map((error: any) => `${error.path || error.instancePath || "<root>"} ${error.message}`);
    throw new Error(`Parent attestation does not match its strict schema: ${paths.join(", ")}`);
  }
